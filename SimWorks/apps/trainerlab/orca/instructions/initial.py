...
class InitialResponseMixin(NsMixin, BaseInstruction):
    ...
    instruction = (
        ...
        "- Each recommended intervention belongs to exactly one problem.\n"
        "- A problem may reference only recommendations whose `target_problem_ref` equals that "
        "problem's `temp_id`.\n"
        "- Every `problems[*].recommendation_refs` entry must exactly equal a "
        "`recommended_interventions[*].temp_id` value.\n"
        "- Do not reuse a recommendation `temp_id` across multiple problems.\n"
        "- If two related problems need similar care, create separate recommendation objects for "
        "each problem.\n"
