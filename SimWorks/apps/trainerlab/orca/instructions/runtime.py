...
class TrainerRuntimeContractInstruction(NsMixin, BaseInstruction):
    instruction = (
        ...
        "snapshot.conditions entries reflect the Problem-level state:\n"
        "- domain_event_id: Problem ID — use as target_event_id for updates/resolves.\n"
        "- kind: 'injury' | 'illness' | 'other'\n"
        "- march_category, severity, control_state ('uncontrolled'|'controlled'|'resolved'), "
        "is_treated (boolean, instructor-set — do NOT set this), is_resolved (boolean).\n"
        "- injury_location, injury_kind (for injury kind); name (for illness kind).\n\n"
