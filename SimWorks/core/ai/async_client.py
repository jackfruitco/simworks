...
from core.ai.parser import OpenAIResponseParser, _get_system_user
...
class AsyncOpenAIChatService:
    ...
    async def generate_patient_reply(self, user_msg: Message) -> List[Message]:
        """
        ...
        payload = await build_patient_reply_payload(user_msg)  # Build the payload for the API request
        ...
        await sync_to_async(user_msg.set_openai_id)(response.id)  # Set the OpenAI response ID to the user message

        parser = OpenAIResponseParser(user_msg.simulation, system_user)  # Initialize the response parser
        return await parser.parse_full_response(response.output_text, response.id)

    async def generate_patient_intro(self, simulation: Simulation) -> List[Message]:
        """
        ...
        input_payload = await build_patient_intro_payload(simulation, prompt)  # Build the payload for the introduction
        response = await self.client.responses.create(
            model=self.model,
            input=input_payload,
        )
