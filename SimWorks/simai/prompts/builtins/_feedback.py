...
class FeedbackModifier:
    ...
    def get(self, kind=None):
        return self._base + self._extras.get(kind, "")

    @classmethod
    def kinds(cls):
        return ["default", "endex", "pausex", "azimuth"]

for kind in FeedbackModifier.kinds():
    label = f"Feedback.{kind}" if kind != "default" else "Feedback"
    modifiers.register(label, lambda u, r, k=kind: FeedbackModifier(u, r).get(k))
