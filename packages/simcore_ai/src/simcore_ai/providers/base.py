...
class BaseProvider(ABC):
    ...
    # ---------------------------------------------------------------------
    def build_final_schema(self, req: "LLMRequest") -> None:
        """
        Compile adapters for the request's output schema class and attach the final provider-ready
        payload back onto `req.output_schema`.

        Flow:
          - Read `req.response_format_cls`as the source-of-truth
          - Apply provider-specific adapters via the central compiler
          - Wrap into provider-specific envelope via `_wrap_schema(meta)`
          - Attach the result to `req.response_format` (final payload used by provider.call)
        """
        try:
            ...
            if wrapped is not None and not isinstance(wrapped, dict):
                raise TypeError(f"{self.name}._wrap_schema must return dict|None, got {type(wrapped).__name__}")
    ...
    def _apply_schema_adapters(self, req: "LLMRequest") -> dict | None:
        """
        Compile the app's Pydantic model (or dict) into a provider-compatible JSON Schema.
        Returns the compiled dict or None if no schema was provided.
        """
        ...
        if not isinstance(base_schema, dict):
            raise TypeError("output_schema_cls must be a Pydantic model class or a JSON Schema dict")
        ...
        if not isinstance(compiled, dict):
            raise TypeError("compile_schema must return a dict JSON Schema")
        return compiled
    ...
    class ToolAdapter(Protocol):
        def to_provider(self, tool: LLMToolSpec) -> Any: ...
        def from_provider(self, tool: Any) -> LLMToolSpec: ...
    ...
    def _tools_to_provider(self, tools: Optional[list[LLMToolSpec]]) -> Optional[list[Any]]:
        """Convert our DTO LLMToolSpec list into provider-native tool specs.
    ...
    def _tools_from_provider(self, provider_tools: Optional[Iterable[Any]]) -> list[LLMToolSpec]:
        """Convert provider-native tool specs into our LLMToolSpec DTOs.
