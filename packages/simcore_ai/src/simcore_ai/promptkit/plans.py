...
Design goals:
- Keep it provider-agnostic and app-agnostic (no Django dependencies here).
...
from typing import Any, Iterable, Iterator, Sequence, TypeVar, Union, overload
import logging
...
def describe_section_spec(spec: object) -> str:
    """
    ...
    """
    try:
        if _is_section_class(spec):
            cls: type[PromptSection] = spec  # type: ignore[assignment]
            ident: Identity = getattr(cls, "get_cls_identity", None)
            ident_str = ident().to_string() if callable(ident) else cls.__name__
            weight = getattr(cls, "weight", None)
            return f"<class> {ident_str} (weight={weight})"
        if _is_section_instance(spec):
            inst: PromptSection = spec  # type: ignore[assignment]
        ...
        if isinstance(spec, dict):
            return f"<invalid> dict keys={sorted(spec.keys())!r}"
        if isinstance(spec, (list, tuple)):
            return f"<invalid> {type(spec).__name__} len={len(spec)}"
        return f"<invalid> {type(spec).__name__}"
...
class PromptPlan:
    ...
    items: list[SectionSpec] = field(default_factory=list)
    ...
    @classmethod
    def from_sections(
        cls, sections: Iterable[SectionSpec], *, context: dict[str, Any] | None = None, meta: dict[str, Any] | None = None
    ) -> "PromptPlan":
    ...
    @classmethod
    def from_section(
            cls, sections: Iterable[SectionSpec], *, context: dict[str, Any] | None = None,
    ...
    def to_debug_list(self) -> list[str]:
        ...
        return [describe_section_spec(s) for s in self.items]

    # ---------- iteration / container protocol ---------------------------

    def __iter__(self) -> Iterator[SectionSpec]:
    ...
    def as_engine_input(self) -> list[SectionSpec]:
        """
