...
"""
Django Schema Registry (class registry).

This registry stores **schema classes** keyed by the finalized identity
...
class SchemaRegistry:
    """Registry of schema **classes** keyed by `(namespace, kind, name)` tuples."""
    ...
    def maybe_register(self, identity: Tuple[str, str, str], cls: Type[Any]) -> None:
        """
        Register a schema class if not already registered.
        ...
        with self._lock:
            ...
            if prev == key:
                logger.debug("schema.duplicate-class-same-identity %s %s", cls, key)
                return
            ...
            if existing is None:
                ...
                logger.debug("schema.registered %s", ".".join(key))
                return
            ...
            msg = f"Schema identity collision {key} between {existing} and {cls}"
            if self._strict:
                logger.error("schema.collision %s", msg)
                raise IdentityCollisionError(msg)
            else:
                logger.warning("schema.collision %s", msg)
