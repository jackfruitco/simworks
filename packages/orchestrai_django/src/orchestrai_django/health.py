...
Goals
-----
- Verify at startup that each registered client can reach its backend with the
...
- Remain backend-agnostic, with optional backend-specific checks for richer coverage.

Design
------
- `healthcheck_client(...)` is the main entrypoint. It performs a minimal request
  when a backend-specific check is available; otherwise it falls back to static
...
from orchestrai.components.providerkit.provider import BaseProvider  # type: ignore
...
async def _provider_healthcheck_async(provider: BaseProvider, *, timeout_s: Optional[float]) -> Tuple[bool, str]:
    """
    Call the backend's own healthcheck coroutine.
    ...
    except Exception as exc:  # pragma: no cover - network and backend errors vary
...
def healthcheck_client(client: OrcaClient, *, timeout_s: Optional[float] = None) -> Tuple[bool, str]:
    """
...
def healthcheck_all_registered(*, timeout_s: Optional[float] = None) -> Dict[str, Tuple[bool, str]]:
    ...
    Args:
        timeout_s: Optional per-call timeout override applied to each backend-specific check.
