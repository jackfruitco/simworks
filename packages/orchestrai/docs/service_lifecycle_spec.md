...
2. **Registration**
   ...
   - Global: decorators register into identity domains (`services`, `instructions`, `response processors`, `schemas`).
...
3. **Resolution**
   ...
   - Identity-based resolution for response processors/schemas/instructions via `Identity.resolve.try_for_(domain, identity)` backed by global registries.
4. **Preparation**
   ...
   - Response processor selection precedence: per-call override (`response processor=`) -> explicit `response_processor_cls` (arg or class) -> `select_response_processors()` (identity + provider/response_schema match).
...
- **Response processor**: runtime `response processor=` override -> explicit `response_processor_cls` (arg/class) -> registry match via `select_response_processors()`.
...
### Error Surfaces
- Missing required context, model/api key configuration errors, instruction render failures (captured defensively), response processor/schema resolution errors, and backend/client issues surface as configuration/build/runtime exceptions.
