# orchestrai/components/codec/base.py
import base64
import json
import logging
from abc import ABC
...
class BaseCodec(IdentityMixin, BaseComponent, ABC):
    ...
    • During encode, set structured-output hints on the request:
        ...
        - provider_response_format (backend-native response_format payload)
    • During decode, validate normalized responses/stream chunks to that schema.
    • Never call backend SDKs directly; providers consume `provider_response_format`
    ...
    def __init_subclass__(cls, **kw) -> None:  # pragma: no cover - light guardrails
        ...
        if adapters:
            cls.schema_adapters = list(sort_adapters(adapters))

    @classmethod
    def _normalize_provider_name(cls, provider: Any) -> str:
        """
        Normalize a provider reference (object or string) into a backend name.

        - If a plain string is passed, return it as-is.
        - If an object with a string `.name` attribute is passed, use that.
        - Otherwise, fall back to `str(provider)`.
        """
    ...
    def matches(cls, *, provider: Any, api: str, result_type: str) -> bool:
        """Return True if this codec should be used for the given backend/api/result_type.
    ...
    def select_for(cls, *, provider: Any, api: str, result_type: str):
        ...
        first pre-filters by backend namespace, then delegates to each codec's
        ...
        def _pred(candidate) -> bool:
            # Quick pre-filter by backend namespace to avoid unnecessary work.
            logger.debug("---- checking codec `%s` against service `%s`",
                         candidate.identity.as_str, f"{provider_name}.{api}.{result_type}"
                         )
            try:
    ...
    async def aencode(self, req: Request) -> None:
        """Attach structured-output hints and backend response format to the request.
        ...
        Sets (when `response_schema` is defined):
          ...
          - req.provider_response_format  (backend-native payload)
        """
        cls = type(self)
    ...
    def _build_provider_response_format(
        ...
        backend's `response_format` payload.
        """
