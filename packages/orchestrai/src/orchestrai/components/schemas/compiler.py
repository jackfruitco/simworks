# orchestrai/schemas/compiler.py
...
backend-specific codec classes instead of using this global registry.
"""
import warnings

warnings.warn("orchestrai.schemas.compiler is deprecated; use codecs instead.", DeprecationWarning, stacklevel=2)

raise DeprecationWarning("orchestrai.schemas.compiler is deprecated; use codecs instead.")
...
# Legacy backend-wide adapter registry --------------------------------------
...
def register_adapter(provider_name: ProviderName, adapter: SchemaAdapter, order: int = 100) -> None:
    """DEPRECATED: register a SchemaAdapter for this backend.

    This maintains a global adapter registry per backend to support legacy
    ...
    backend-specific codec classes instead of registering adapters here.
    """
    warnings.warn(
        "orchestrai.schemas.compiler.register_adapter is deprecated; "
...
def compile_schema(schema: dict, *, provider: ProviderName) -> dict:
    """DEPRECATED: compile a schema for a given backend.

    Runs all registered adapters for the backend (in ascending order) and
    ...
    """
    warnings.warn(
        "orchestrai.schemas.compiler.compile_schema is deprecated; "
