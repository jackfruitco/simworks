...
from orchestrai.identity.domains import SCHEMAS_DOMAIN, SERVICES_DOMAIN, normalize_domain
...
@pytest.mark.parametrize(
    ...
    [
        ("SCHEMAS", "AttrDomain", "schemas", "arg"),
        (None, "SCHEMAS", "schemas", "attr"),
    ],
)
...
def test_registry_collision_is_domain_sensitive_and_mentions_both_classes():
    ...
    class DemoSchema:
        identity = Identity(domain=SCHEMAS_DOMAIN, namespace="demo", group="svc", name="item")

    registry.register(DemoService)
    registry.register(DemoSchema)
